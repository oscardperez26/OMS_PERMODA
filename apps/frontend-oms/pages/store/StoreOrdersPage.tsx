...
import {
  mapOrdersListItemToStoreVm,
  STORE_LIST_INTEGRATION_WARNING,
  type StoreOrderListItemVm,
...
export function StoreOrdersPage() {
  ...
  const [orders, setOrders] = useState<StoreOrderListItemVm[]>([]);
  ...
  useEffect(() => {
    ...
    async function loadStoreOrders() {
      ...
      setLoadError("");
      try {
        const apiOrders = await listOrders(accessToken);
        setOrders(
          apiOrders.map((item) => mapOrdersListItemToStoreVm(item, user?.storeId)),
        );
      } catch (error) {
        const message =
          error instanceof Error
            ? error.message
            : "No se pudieron cargar pedidos de tienda";
  ...
  }, [accessToken, isAuthLoading, user?.storeId]);
  ...
  const filteredOrders = useMemo(() => {
    return orders.filter((o) => {
      ...
      const matchTransporter =
        filters.transporter === "0" || o.transporterCode === filters.transporter;
      ...
      if (activeTab === "Pendiente") {
        matchTab =
          o.statusLabel === "Confirmado" || o.statusLabel === "Preparacion en curso";
        ...
        matchTab =
          o.statusLabel === "Cancelado" || o.statusLabel === "Con novedad";
      }
  ...
  const goToDetail = (order: StoreOrderListItemVm) =>
    navigate(`/tienda/orders/${order.orderId}`);

  const goToTicket = (order: StoreOrderListItemVm, e: ReactMouseEvent) => {
    e.stopPropagation();
  ...
  return (
    <div className="w-100">
      <div className="mb-4">
        <h1 className="m-0 fw-bold text-dark h4 mb-3">
          Listado de pedidos - <span style={{ color: "blue" }}>GESTOR</span>
        </h1>
        ...
        {isLoadingOrders && (
          <div className="alert alert-info py-2 px-3 small mb-2">
            Cargando pedidos de tienda...
          </div>
        )}
        {loadError && (
          <div className="alert alert-danger py-2 px-3 small mb-2">{loadError}</div>
        )}

        <div
          className="bg-white p-2 border border-light rounded mb-2"
          style={{ boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }}
        >
          <div className="d-flex gap-2 flex-wrap align-items-center">
            <div className="d-flex border rounded overflow-hidden" style={{ height: "30px" }}>
              <div
                className="bg-light px-2 d-flex align-items-center border-end small text-muted"
            ...
            >
              <i className="bi bi-search text-muted me-2" style={{ fontSize: "0.75rem" }} />
              <input
            ...
            <div className="date-range-container position-relative">
              ...
              >
                <i className="bi bi-calendar3 text-muted me-2" style={{ fontSize: "0.75rem" }} />
              ...
              {showDatePicker && (
                ...
                >
                  <div className="d-flex flex-column gap-3">
                    <div>
                      <label className="koaj-label mb-1" style={{ fontSize: "0.65rem" }}>
                        Desde
                      </label>
                      <input
                        type="date"
            ...
            >
              ...
              {STATUS_OPTIONS.filter((o) => o.value !== "0").map((o) => (
            ...
            >
              ...
              {ORIGIN_OPTIONS.filter((o) => o.value !== "0").map((o) => (
                <option key={o.value} value={o.value}>
                  {o.label}
                </option>
              ))}
            ...
            >
              ...
              {TRANSPORTER_OPTIONS.filter((o) => o.value !== "0").map((o) => (
                <option key={o.value} value={o.value}>
                  {o.label}
                </option>
              ))}
            ...
            >
              ...
              {STORE_OPTIONS.filter((o) => o.value !== "0").map((o) => (
                <option key={o.value} value={o.value}>
                  {o.label}
                </option>
              ))}
      ...
      <div className="d-flex border-bottom mb-0 bg-white px-3 flex-wrap">
        {[
          ...
          <button
            key={tab.label}
            className={`btn border-0 rounded-0 px-3 py-3 d-flex align-items-center gap-2 small fw-medium ${
              activeTab === tab.label
                ? "border-primary border-bottom border-3 text-primary bg-light bg-opacity-10"
                : "text-muted"
            }`}
        ...
        <table className="koaj-table">
          <thead style={{ backgroundColor: "#003366" }}>
            <tr className="orders-head-row">
              ...
              <th className="fw-bold text-white small" style={{ width: "120px" }}>
                Asignacion
              </th>
              <th className="fw-bold text-white small" style={{ width: "100px" }}>
                Id pedido
              </th>
              <th className="fw-bold text-white small" style={{ width: "120px" }}>
                Referencia
              ...
              <th className="fw-bold text-white small" style={{ width: "80px" }}>
                Tienda
              </th>
              <th className="fw-bold text-white small" style={{ width: "150px" }}>
                Fecha
              </th>
              <th className="fw-bold text-white small" style={{ width: "130px" }}>
                Origen
              </th>
              <th className="fw-bold text-white small" style={{ width: "130px" }}>
                Servicio
              ...
              <th className="text-center text-white" style={{ width: "100px" }}>
                Acciones
              </th>
            </tr>
          </thead>
            ...
            {filteredOrders.length === 0 ? (
              <tr>
                <td colSpan={11} className="text-center text-muted py-5">
                  No se encontraron pedidos en la categoria <strong>{activeTab}</strong>.
                </td>
              </tr>
            ) : (
